...
- **Testes:** descrições de `describe`/`it` em português; `it` sempre no padrão imperativo **"deve…"** (ex.: `it('deve retornar 404 quando o recurso não existe')`).
- **Nomenclatura de arquivos:** sempre **kebab-case** (ex.: `painel-exemplo.tsx`, `exemplo.controller.ts`).
